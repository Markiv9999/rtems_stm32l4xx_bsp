...
int mspi_init(void) {
  ...
  *   the function MUST be configured in accordance with the system configuration 
  */

  /* Enables the clock for the gpios, sets to alternate function and configures the alternate function */
  mspi_gpio_cfg();
...
u16 mspi_transfer_dma( struct mspi_cmd (*device_fun_handler)(void*),struct mspi_interface interface,void* argument){
  ...
  // if read functional mode enable the dma pull channel before sending the address
  if (cmd.data_mode > 0){
  ...
  if (cmd.addr_mode > 0 ){
    QUADSPI->AR  |=  ( cmd.addr_cmd << QUADSPI_AR_ADDRESS_Pos); //for now even plane, beginning of page
  }
